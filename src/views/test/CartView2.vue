...
const loadCarts = async () => {
  ...
  try {
    await cartStore.fetchCarts(Number(userId));
...
const handlePaymentConfirmation = async () => {
  ...
  if (!confirmPayment.value) {
    try {
      ...
      const orderData = {
        postCode: "39021",              // 실제 우편번호
        address: "서울시 강남구",        // 주소
        detailAddress: "아파트 101동 202호", // 상세주소
        ordererName: userStore.user.name || "홍길동", // 로그인 유저 이름
        phoneNumber: "010-1234-5678",   // 실제 유저 번호
        payMethod: "EASY_PAY"              // Enum 값 (PayMethod에 정의된 것 중 하나)
      };
