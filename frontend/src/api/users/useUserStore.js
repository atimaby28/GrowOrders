...
export const useUserStore = defineStore('user', () => {
  ...
  const setWithEncrypt = (data, rememberMe) => {
    ...
    if (rememberMe) {
      encryptLocalStorage.setItem(key, JSON.stringify(data));
    } else {
      encryptSessionStorage.setItem(key, JSON.stringify(data));
    }
  };
  ...
  return { isLogin, user, checkLogin, setWithEncrypt, logout, getUserId };
});
